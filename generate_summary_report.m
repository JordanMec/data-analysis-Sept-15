...
if ~isempty(efficacyScoreTable)
    ...
    for i = 1:topN
        ...
        fprintf(fid, '| #%d | %s-%s-%s | %.1f | %.1f | %s | %.1f | %.1f |\n', ...
            row.rank, row.location{1}, row.filterType{1}, row.mode{1}, ...
            row.mean_efficacy_score, row.score_range_half, ...
    ...
    fprintf(fid, '- **Best Overall Configuration**: %s-%s-%s (Score: %.1f)\n', ...
        bestConfig.location{1}, bestConfig.filterType{1}, bestConfig.mode{1}, ...
        bestConfig.mean_efficacy_score);
...
if exist('activeAnalysisResults', 'var') && ~isempty(activeAnalysisResults)
    ...
    if isfield(activeAnalysisResults, 'ioRatios')
        ...
        for i = 1:length(ioConfigs)
            ...
            pm25_range = abs(diff(ioData.stats.pm25_range));
            fprintf(fid, '| %s | %.3f | %.3f | Range %.1f%% |\n', ...
                strrep(config, '_', '-'), ...
                ioData.stats.pm25_mean, ...
                ioData.stats.pm10_mean, ...
                100 * pm25_range / (2 * ioData.stats.pm25_mean));
        end
    end
    ...
    if isfield(activeAnalysisResults, 'triggerResponse')
        ...
        for i = 1:length(triggerConfigs)
            ...
            if isfield(triggerData, 'metrics') && isfield(triggerData, 'pm25_response')
                fprintf(fid, '| %s | %.1f hours | %.0f%% | %.1f%% |\n', ...
                    strrep(config, '_', '-'), ...
                    triggerData.metrics.avg_response_time, ...
                    triggerData.metrics.active_percentage, ...
                    triggerData.pm25_response.avg_peak_reduction);
            end
        end
    end
    ...
    if isfield(activeAnalysisResults, 'ioRatios')
        ...
        fprintf(fid, '- **Best I/O Ratio**: %s (%.3f)\n', strrep(bestConfig, '_', '-'), bestIO);
    end
...
for m = 1:numel(modes)
    ...
    if ~isempty(modeRows)
        fprintf(fid, '### %s Mode\n', modes{m});
        fprintf(fid, '- Average PM2.5 Reduction: %.1f%%\n', mean(modeRows.percent_PM25_reduction));
        fprintf(fid, '- Average Annual Cost: $%.2f\n', mean(modeRows.total_cost));
        fprintf(fid, '- Average AQI Hours Avoided: %.0f\n\n', mean(modeRows.AQI_hours_avoided));
    end
end
