...
exampleIdx = find(strcmp(scenarios.location,'adams') & ...
    (strcmpi(scenarios.filterType,'hepa') | strcmpi(scenarios.filterType,'hepa 13')) & ...
    strcmp(scenarios.mode,'active'), 1);

if ~isempty(exampleIdx)
    filterTarget = scenarios.filterType{exampleIdx};
    if any(strcmpi(filterTarget, {'hepa', 'hepa 13'}))
        summaryFilterMask = strcmpi(summaryTable.filterType, 'hepa') | strcmpi(summaryTable.filterType, 'hepa 13');
    elseif any(strcmpi(filterTarget, {'merv', 'merv 15'}))
        summaryFilterMask = strcmpi(summaryTable.filterType, 'merv') | strcmpi(summaryTable.filterType, 'merv 15');
    else
        summaryFilterMask = strcmpi(summaryTable.filterType, filterTarget);
    end
    ...
    if ~isempty(tightRow) && ~isempty(leakyRow)
        ...
        title(sprintf('Physical Bounds Evolution for %s %s %s During First Week', ...
            ...
            format_filter_label(scenarios.filterType{exampleIdx}), ...
